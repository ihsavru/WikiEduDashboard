...
const fetchUploadMetadata = (uploads) => {
  let url = `http://commons.wikimedia.org/w/api.php?action=query&format=json&titles=`;
  _.forEach(uploads, upload => {
    url = `${url}File:${upload.file_name}|`;
  });
  url = url.slice(0, -1);
  return new Promise((res, rej) => {
    return $.ajax({
...
export const setUploadMetadata = (uploads) => dispatch => {
  const firstBatch = uploads.slice(0, 50);
  const secondBatch = uploads.slice(50, 100);
  return (
    Promise.all([fetchUploadMetadata(firstBatch), fetchUploadMetadata(secondBatch)])
    .then(resp => dispatch({
